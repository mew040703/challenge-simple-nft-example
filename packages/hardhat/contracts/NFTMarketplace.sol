...
contract NFTMarketplace is ReentrancyGuard, Ownable, IERC721Receiver {
    ...
    function cancelListing(uint256 listingId) external nonReentrant {
        ...
        require(tokenToListing[listing.nftContract][listing.tokenId] == listingId, "Listing not active");
        require(
    ...
    function getListingsBySeller(address seller) external view returns (ListingWithId[] memory) {
        uint256 count = 0;
        for (uint256 i = 1; i < nextListingId; i++) {
            Listing storage l = listings[i];
            if (l.seller == seller && tokenToListing[l.nftContract][l.tokenId] == i) {
                count++;
            }
        }

        ListingWithId[] memory arr = new ListingWithId[](count);
        uint256 idx = 0;
        for (uint256 i = 1; i < nextListingId; i++) {
            Listing storage l = listings[i];
            if (l.seller == seller && tokenToListing[l.nftContract][l.tokenId] == i) {
                arr[idx++] = ListingWithId({
                    listingId: i,
                    tokenId: l.tokenId,
